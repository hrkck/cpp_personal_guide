...
template <typename T> void display(deque<T> deqoy);
template <typename T> int find(deque<T> deqoy, T value);
...
// Finds any given value in items of a deque, prints the position of the item in
// the deque if the value is found. if the value is not found, returns -1.
template <typename T> int find(deque<T> deqoy, T value) {
  ...
  // Looping through the "uppercase_deqoy" and searching for the
  // "uppercase_value"
  for (size_t i = 0; i < uppercase_deqoy.size(); i++) {
